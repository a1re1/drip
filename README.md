...
script), seeded on first run with a water-droplet mascot and a short info
block:
...
```
  ...
  ~/src/drip
  session 4f2a1c9d
  model profiles (active: default)
    * default: anthropic/claude-sonnet-4-5
      fast: anthropic/claude-haiku-4-5 (quick)
```
...
- Output is plain text: escape sequences and control characters are stripped
  before it reaches the timeline, so a banner can never move the cursor, clear
  the screen or set a title. The script runs as local code, exactly like
  `statusLine`; nothing reads or executes `~/.claude` settings.
