...
file they were loaded from, and every consumer — the model request and the TUI's
system-prompt picker — resolves the directories plus any entry still left in
`runtime.system_prompt_profiles`, with the inline entry winning on an id
...
a `-2` suffix directory. A directory that already names the id but carries no
prompt at all — no `prompt.md` and no inline `prompt` — takes the setting's
prompt into its `prompt.md` first, so clearing the setting never drops the only
copy of that prose. An entry that cannot be placed (a directory whose
...
script), seeded on first run with a rainbow rain-cloud mascot and a single
...
In a terminal the cloud is painted through a blue → cyan → violet ramp and
the info line is color-coded (accent `drip`, cyan version, violet profile, dim
separators, light path). The mascot used to be a water droplet; an unedited
`startup-message.sh` from that version is upgraded to the rain cloud on the
next start (see below), while an edited one is left alone.
...
- A `startup-message.sh` still byte-identical to the default an earlier version
  shipped — including the earlier water-droplet banners — has never been edited,
  so it is upgraded to the current default on the next start. Once you edit it,
  drip never touches it again.
