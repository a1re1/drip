...
pub fn format_transcript_entry_line(entry: &TranscriptEntry) -> String {
    match entry {
        ...
        TranscriptEntry::RunEnd(entry) => format!(
            ...
            serde_json::to_value(&entry.reason)
                .unwrap()
                .as_str()
                .unwrap(),
            entry.iterations
...
mod tests {
    ...
    fn test_format_transcript_entry_line() {
        ...
        assert_eq!(
            format_transcript_entry_line(&entry),
            "[  7] tool-call ran bash"
        );
        ...
        assert!(
            line.starts_with("=== "),
            "model line should reuse formatModelRoute: {line}"
        );
