...
fn record_event(
    report: &mut InspectGoalReport,
    entry: &crate::cli::transcript::TranscriptEventEntry,
) {
    *report
        .event_counts
    ...
    if entry.kind == HarnessEventType::RateLimited {
        ...
        let wait = entry
            ...
            .unwrap_or_else(|| rx_waiting_seconds(&entry.detail).unwrap_or(0.0));
    ...
    for entry in &entries {
        match entry {
            TranscriptEntry::Goal(goal) => {
                goals.insert(
                    goal.goal_id.clone(),
                    new_goal_report(&goal.goal_id, &goal.text, &goal.at),
                );
            }
            ...
            TranscriptEntry::RunEnd(run_end) => {
                if let Some(report) = goals.get_mut(&run_end.goal_id) {
                    ...
                    let wall_seconds = match (
                        parse_epoch_ms(&report.started_at),
                        parse_epoch_ms(&run_end.at),
                    ) {
                        (Some(start_ms), Some(end_ms)) => {
                            Some(((end_ms - start_ms) as f64 / 1000.0).round().max(0.0) as i64)
                        }
                        _ => None,
                    };
    ...
    let (completion_anchor, expectations, anomalies) = match state.as_ref() {
        Some(state) => (
            state
                .completion_anchor
                .as_ref()
                .map(|_| crate::core::state::describe_completion_anchor(state)),
            crate::core::state::describe_expectations(state)
                .lines()
                .map(String::from)
                .collect(),
...
pub fn format_inspect_report(report: &InspectReport) -> String {
    ...
    for goal in &report.goals {
        lines.push(format!(
            "goal {}: {}",
            &goal.goal_id.chars().take(8).collect::<String>(),
            goal.goal
        ));
        lines.push(format!(
            "  {}{} — started {}",
        ...
        let tool_lines: Vec<String> = goal
            ...
            .map(|(name, stat)| {
                let duration = if stat.total_duration_ms > 0 {
                    format!(
                        ", {}s in-tool",
                        (stat.total_duration_ms as f64 / 1000.0).round() as i64
                    )
                } else {
                    String::new()
                };
        ...
        if goal.operator_messages.count > 0 {
            ...
            let avg = if !latencies.is_empty() {
                ...
                format!(
                    " (adoption avg {}s)",
                    (sum as f64 / latencies.len() as f64 / 1000.0).round() as i64
                )
            } else {
                String::new()
            };
            ...
            lines.push(format!(
                "  steering: {} message(s){avg}",
                goal.operator_messages.count
            ));
        }
    ...
    if report.completion_anchor.is_some()
        || !report.expectations.is_empty()
...
mod tests {
    ...
    fn make_temp_root(prefix: &str) -> PathBuf {
        ...
        let root = std::env::temp_dir().join(format!("{}{}-{}", prefix, std::process::id(), id));
