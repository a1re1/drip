...
pub fn load_roles_from_file(file_path: &str) -> Result<RoleSetupSource> {
    let raw = std::fs::read_to_string(file_path).with_context(|| format!("reading {file_path}"))?;
...
fn normalize_bindings(value: Option<&serde_json::Value>) -> Option<HarnessRoleBindings> {
    ...
    (planning.is_some() || replanning.is_some() || task.is_some()).then_some(HarnessRoleBindings {
        planning,
        replanning,
        task,
    })
    ...
    match serde_json::from_str::<serde_json::Value>(raw_profiles) {
        Ok(parsed_value) => {
            if let Some(entries) = parsed_value.as_array() {
                ...
                issues.push(
                    "config role_profiles: expected a JSON array of role definitions".to_string(),
                );
            }
        }
        Err(_) => {
...
pub fn planning_effort_default(
    is_planning: bool,
    explicit: bool,
    current: Option<&str>,
) -> Option<&'static str> {
    (is_planning && !explicit && (current == Some("high") || current.is_none()))
        .then_some(PLANNING_DEFAULT_REASONING_EFFORT)
}

fn overlay_into(
    definitions: &mut indexmap::IndexMap<String, RoleDefinition>,
    role: RoleDefinition,
) {
...
fn merge_role_definitions(
    ...
    indexmap::IndexMap<String, RoleDefinition>,
    RoleSetupSource,
    RoleSetupSource,
) {
...
pub fn resolve_role_setup(args: &ResolveRoleSetupArgs) -> ResolvedRoleSetup {
    ...
    let mcp_configured_names: HashSet<&str> =
        args.mcp_server_names.iter().map(String::as_str).collect();
    ...
    for role in roles.iter_mut() {
        ...
        if let Some(route) = role.route.as_mut() {
            if let Some(effort) =
                planning_effort_default(is_planning, explicit, route.reasoning_effort.as_deref())
            {
...
fn resolve_model_profile_route(
    ...
    fn convert(route: &crate::core::inference::ResolvedModelRoute) -> ModelRoute {
        ModelRoute {
            fallback_route: route
                .fallback_route
                .as_ref()
                .map(|inner| Box::new(convert(inner))),
    ...
    let resolved =
        crate::core::inference::resolve_model_profile_route(settings, model_profile_id, env)?;
...
mod overlay_tests {
    ...
    fn planning_roles_step_down_from_high_unless_they_set_an_effort() {
        assert_eq!(
            planning_effort_default(true, false, Some("high")),
            Some("medium")
        );
...
mod tests {
    ...
    fn quantified_goal_guidance_is_present_in_planning_prompts() {
        assert!(planner_role()
            ...
            .as_deref()
            .unwrap_or_default()
            .contains("quantifies over an input space"));
        assert!(planner_role()
            .prompt
            .as_deref()
            .unwrap_or_default()
            .contains("constructs variant inputs"));
        assert!(architect_role()
            .prompt
            .as_deref()
            .unwrap_or_default()
            .contains("quantifies over an input space"));
