...
pub fn build_run_record(args: &BuildRunRecordArgs) -> RunRecord {
    ...
    RunRecord {
        ...
        anomalies: Some(args.result.state.anomalies.clone())
...
mod tests {
    ...
    fn serialises_role_inference_with_two_roles_and_omits_empty_map() {
        ...
        assert_eq!(
            ...
            serde_json::json!({
                "author": { "calls": 12, "latencyMs": 4800, "completionTokens": 910, "promptTokens": 40000, "cacheReadTokens": 30000, "hedgesFired": 4, "hedgesWon": 1 },
                "planner": { "calls": 3, "latencyMs": 1200, "completionTokens": 210, "promptTokens": 5000, "cacheReadTokens": 4000, "hedgesFired": 0, "hedgesWon": 0 },
            }),
            "{json}"
        );
    }
    ...
    fn captures_the_outcome_fields_a_driver_needs() {
        ...
        let json = serde_json::to_string(&record).unwrap();
        assert!(
            !json.contains("completionAnchor") && !json.contains("anomalies"),
            "{json}"
        );
    }
