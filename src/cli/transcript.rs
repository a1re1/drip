        ...
        let mut fields = match inner {
            ...
            _ => {
                return Err(serde::ser::Error::custom(
                    "transcript entry must serialize to an object",
                ))
            }
        };
        fields.insert(
...
pub const TRANSCRIPT_ENTRY_TYPES: [&str; 7] = [
    "error", "event", "goal", "info", "model", "run-end", "skill",
];
...
pub fn format_model_route_lines(entry: &TranscriptModelEntry) -> Vec<String> {
    ...
    let effort = |value: Option<&str>| -> String {
        value.map(|v| format!(" · effort {v}")).unwrap_or_default()
    };
...
mod tests {
    ...
    fn serializes_with_canonical_key_order() {
        ...
        assert_eq!(
            serde_json::from_str::<TranscriptEntry>(&line).unwrap(),
            entry
        );
    ...
    fn skips_torn_lines_and_unknown_entry_types() {
        ...
        assert!(matches!(&entries[0], TranscriptEntry::Info(note) if note.text == "hello"));
        assert!(fs::read_to_string(&transcript_path)
            .unwrap()
            .contains("mystery"));
        assert_eq!(
            read_transcript(&root.path().join("missing.jsonl")),
            Vec::new()
        );
    }
}
