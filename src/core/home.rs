...
pub const DEFAULT_SUMMARY_PREFERENCES: &str = concat!(
    ...
    "report task counts only when they help the reader, claim a verification\n",
    "passed only when a\n",
    ...
    "- Aim for a one-pager the operator can read in twenty seconds: about twenty\n",
    "  lines, no per-task walkthrough. The full detail lives in the run report.\n",
    "- Open with two to four sentences: one plain sentence on whether the goal was\n",
    "  met, then only the two or three things that matter most, plus anything the\n",
    "  operator must decide.\n",
    "- Then one 'Testing & verification:' line: the strongest recorded check's\n",
    "  command with its executed/passed/failed counts and anchor, and how many\n",
    "  further checks were recorded. Say plainly when none was recorded.\n",
    "- Then at most five short bullets for the notable outcomes and the open items\n",
    "  (blocked, dropped, deferred, unverified), with why.\n",
    "- Finish with a 'Details: report.md' line pointing at the full run report.\n",
    "- Prefer concrete nouns — file paths, commands, counts — over adjectives, and\n",
    "  never repeat a count or command that already appeared.\n",
);
