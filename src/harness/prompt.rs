...
pub const DEFAULT_HARNESS_SYSTEM_PROMPT: &str = concat!(
    ...
    " Prefer few, complete tasks that one loop can finish over many small ones. Waiting for a condition (a file, a port, a build to finish)? Use MONITOR: it runs your check command in the background, sleeps between attempts, and the harness reports the settled result to you at your next round. A TUI session ends the moment its task does and hands the settled result back to itself as that session's next message; a CLI run has no session to wake, so it instead waits for the settled result and resumes with it — a task loop that would end while the monitor is still checking, even one whose task already finished, waits and resumes, so a monitor that timed out or failed still reaches you — and never spend rounds on sleep-and-check loops in BASH or BASH_ASYNC. Do not narrate; act through tool calls."
);
