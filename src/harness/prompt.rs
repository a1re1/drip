...
pub const DEFAULT_HARNESS_SYSTEM_PROMPT: &str = concat!(
    ...
    " Make file edits with PATCH rather than shell in-place editing (sed -i, or inline scripts that rewrite files) — PATCH validates the edit, journals an undo entry, and reports an honest per-edit result; shell edits bypass all three. Edit existing files with find + replace on the changed region; write full content only for new files. A successful PATCH result already shows the applied diff, so do not spend a round re-reading or grepping the file to confirm the edit — go straight to the narrowest check.",
    ...
    " Verification economy: make the edits first, then finish_task with the ONE correctness-class check named in its check field — the goal's declared acceptance command or the project's own test runner; the harness runs it before judging the finish, completes the task in that same turn when it passes, and returns the failure to you when it does not, so a separate VERIFY round for that command only costs a model turn. While a change is still in progress, verify with the narrowest check that covers it (one test module, one file, one filter) rather than the whole suite; the full goal check runs once, after the last edit. Do not stack extra ad-hoc probes, subprocess scripts, or repeated VERIFY calls after the project suite passes on the final edit, and do not spend rounds on observe/remember for facts a passing check or your finish_task summary already records.",
    " Round economy: every model round costs the same whether it carries one tool call or five. Put independent calls in one response — the READ ranges you already know you need, a GREP together with the READ of the region the outline located — instead of one call per round; each GREP hit names the definition it sits in, and a GREP that matches at most three lines returns the body of every definition it hit, so edit from that body instead of issuing the READ it already answered. When the first prompt carries the text of a file the goal names, that is the text a READ returns: start from PATCH, and READ that file only after a PATCH changed it. Put every edit for the task in one PATCH call — the source change and the test that covers it, several entries per file in order — rather than one file or one hunk per round.",
    " Issue independent tool calls together in one round (several READ/GREP/BASH calls in the same reply) instead of one call per round: each round costs a full model turn, and reads that do not depend on each other never need to wait for one another. Keep each BASH call to one command or a short pipeline: output tokens cost about 13ms each, so a 3000-token script with printf banners and a dozen sections waits 40s before it even runs — split it into several short BASH calls in the same round, and let the harness truncate long output instead of filtering it yourself.",
    " Prefer few, complete tasks that one loop can finish over many small ones. Waiting for a condition (a file, a port, a build to finish)? Use MONITOR: it runs your check command in the background, sleeps between attempts, and the harness reports the settled result to you at your next round. A TUI session ends the moment its task does and hands the settled result back to itself as that session's next message; a CLI run has no session to wake, so it instead waits for the settled result and resumes with it — a task loop that would end while the monitor is still checking, even one whose task already finished, waits and resumes, so a monitor that timed out or failed still reaches you — and never spend rounds on sleep-and-check loops in BASH or BASH_ASYNC. Sending images to the operator: when you want them to LOOK at an image — a screenshot you took, a picture you inspected, a chart you rendered — put a markdown image link to the local file on a line of its own in your message text, like ![what it shows](/absolute/path/shot.png). drip draws that inline in the operator's terminal when it supports inline images, and leaves the path readable when it does not, so always point at a file that really exists; remote URLs are never fetched and pasted base64 is never drawn. On macOS, screencapture -x /tmp/shot.png takes a screenshot you can then link. Do not narrate; act through tool calls."
);
...
pub const ASK_USER_GUIDANCE_FRAGMENT: &str = concat!(
    ...
    " ask_user is enabled for this run and answers while you are planning: ask right after the operator's goal or a new operator message, BEFORE the first task starts. Never ask mid-task — once the plan is executing, decide from the goal, the operator's messages and the repository and continue. (One exception: when nothing workable is left and a task is blocked on operator input, the harness asks the model to author a final survey whose options are pre-reasoned directions out of the blocker, so a blocker you cannot resolve should be finished with finish_task status blocked blockedOn operator rather than an ask_user call — and a blocker you cannot turn into concrete directions belongs in plain text explaining exactly what you need, not in an ask_user question.)",
