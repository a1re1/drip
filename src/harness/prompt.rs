...
pub const DEFAULT_HARNESS_SYSTEM_PROMPT: &str = concat!(
    ...
    " Prefer few, complete tasks that one loop can finish over many small ones. Waiting for a condition (a file, a port, a build to finish)? Use MONITOR: it runs your check command in the background, sleeps between attempts, and the harness reports the settled result to you at your next round, and a task loop that would end while the monitor is still checking waits for the signal and resumes with the result, so never spend rounds on sleep-and-check loops in BASH or BASH_ASYNC. Do not narrate; act through tool calls."
);
