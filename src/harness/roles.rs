...
pub fn build_role_map(
    roles: Option<&[HarnessRoleRuntime]>,
) -> IndexMap<String, HarnessRoleRuntime> {
...
pub fn resolve_loop_role(
    ...
    let role_name = match current_task {
        Some(task) => task
            .role
            .clone()
            .or_else(|| bindings.and_then(|b| b.task.clone())),
...
pub fn resolve_planning_role(
    ...
    let role_name = if replanning && !escalate {
        bindings
            .replanning
            .clone()
            .or_else(|| bindings.planning.clone())
    } else {
...
pub fn filter_tools_for_role<T: NamedTool>(
    tools: Vec<T>,
...
mod tests {
    ...
    fn resolve_loop_role_prefers_task_assignment_then_bindings() {
        ...
        assert_eq!(
            resolve_loop_role(&map, Some(&task("t1", Some("reviewer"))), Some(&bindings))
                .map(|r| r.name),
    ...
    fn filter_tools_for_role_restricts_to_allowlist() {
        ...
        assert_eq!(
            filter_tools_for_role(tools.clone(), Some(&role("open"))),
            tools
        );
