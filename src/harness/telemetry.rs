...
use crate::core::types::{
    HarnessState, HarnessTelemetryConfig, PromotedContextEntry, ToolTelemetryRecord,
};
...
fn sort_json_value(value: Value) -> Value {
    match value {
        ...
        Value::Object(map) => {
            ...
            let mut sorted_entries: Vec<(String, Value)> = map
                .into_iter()
                .map(|(key, value)| (key, sort_json_value(value)))
                .collect();
            sorted_entries.sort_by(|(left_key, _), (right_key, _)| {
                crate::tools::helpers::locale_compare(left_key, right_key)
            });
...
pub fn record_tool_telemetry(
    ...
    let truncated_output =
        truncate_text_keeping_ends(args.output, config.max_promoted_output_chars as usize);
    let mut record = match state.telemetry.get(&key) {
        Some(existing_record) => existing_record.clone(),
        None => ToolTelemetryRecord {
            call_count: 0,
            input_preview: truncate_text(
                &canonicalize_tool_input(args.raw_input),
    ...
    record
        .iterations_used
        .retain(|&loop_index| loop_index > state.r#loop - config.recency_window);
    state.telemetry.insert(key.clone(), record.clone());

    if let Some(promoted_entry) = state
        .promoted_context
        .iter_mut()
...
pub fn run_telemetry_maintenance(
    ...
    let telemetry_snapshot: Vec<(String, ToolTelemetryRecord)> = state
        .telemetry
        .iter()
    ...
    for (key, mut record) in telemetry_snapshot {
        ...
        let entry = PromotedContextEntry {
            ...
            last_failed: if record.last_failed == Some(true) {
                Some(true)
            } else {
                None
            },
    ...
    let promoted_keys: Vec<String> = state
        .promoted_context
        .iter()
...
mod tests {
    use super::{
        ...
        run_telemetry_maintenance, tool_telemetry_key, RecordToolTelemetryArgs,
        RunTelemetryMaintenanceArgs, TelemetryMaintenanceResult,
    };
    use crate::core::types::{
        HarnessState, HarnessTelemetryConfig, PromotedContextEntry, DEFAULT_TELEMETRY_CONFIG,
    };
    ...
    fn canonicalizes_tool_inputs_so_key_order_does_not_split_telemetry() {
        assert_eq!(
            canonicalize_tool_input(r#"{"b":1,"a":{"d":2,"c":3}}"#),
            r#"{"a":{"c":3,"d":2},"b":1}"#
        );
        assert_eq!(canonicalize_tool_input("not json"), "not json");
        assert_eq!(
            tool_telemetry_key("READ", r#"{"b":1,"a":2}"#),
            tool_telemetry_key("READ", r#"{"a":2,"b":1}"#)
        );
    }
    ...
    fn decays_unused_promoted_entries_and_expires_them_at_zero_ttl() {
        ...
        record_call(
            &mut state,
            "READ",
            r#"{"core/":"src/a.ts"}"#.replace("core/", "path").as_str(),
            &config,
        );
        let _ = run_telemetry_maintenance(&mut state, empty_dynamic(), &config);
        assert_eq!(state.promoted_context[0].ttl, config.base_ttl);
    ...
    fn truncates_oversized_outputs_keeping_both_ends_and_canonicalizes_nested_arrays() {
        ...
        let record = state
            ...
            .values()
            .next()
            .expect("telemetry record exists");
