...
/// col_end)`.
///
/// Mirrors exactly the layout `render_frame` uses (portrait stacks them
/// full-width; landscape seats them in the left column), so a click's cell can
/// be mapped back to the list row it landed on. Empty when the terminal is too
/// small to paint the frame at all.
pub fn panel_regions(vm: &WatchViewModel, cols: usize, rows: usize) -> Vec<(usize, usize, usize, usize)> {
    ...
    let heights: Vec<usize> = if cols < PORTRAIT_MAX_COLS {
        portrait_heights(body_h, &[vm.sessions.len(), vm.tasks.len(), vm.shells.len(), 0])[..3].to_vec()
    } else {
        let sessions_desired = vm.sessions.len().max(1) + 2;
        let shells_desired = vm.shells.len().max(1) + 2;
        let mut h1 = sessions_desired.min(MIN_PANE.max(body_h.saturating_sub(2 * MIN_PANE)));
        ...
        vec![h1, h2.max(0) as usize, h3]
    };

    let width = if cols < PORTRAIT_MAX_COLS { cols } else { (cols * 2 / 5).max(30).min(cols - 20) };
    let mut out = Vec::new();
    let mut top = 0usize;
    for h in heights {
        out.push((top + 1, top + h, 1, width));
        top += h;
    }
    out
}
