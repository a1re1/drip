...
fn one_exchange() -> Vec<TranscriptEntry> {
    vec![
        ...
        inference(
            1,
            "z-ai/glm-5.3-flash — 21018 prompt (0 cached, 0 written), 864 completion",
        ),
...
fn read_result_infer_patch_result_folds_into_one_two_tool_row() {
    ...
    assert!(
        rows.iter().any(|r| r.contains("fix the flaky test")),
        "{rows:?}"
    );
    assert!(
        rows.iter()
...
fn next_cycle_begins_fresh_row_with_numbered_task_preview() {
    ...
    let transition = rows
        .iter()
        .find(|r| r.contains("cycle 2/5"))
        .expect("transition row");
    assert!(
        transition.contains("task-1: fix the flaky test"),
        "{transition}"
    );
    assert!(
        transition.contains("budget 100/60000 tokens"),
        "{transition}"
    );
    assert!(
...
fn empty_cycle_produces_no_group_row() {
    let entries = vec![
        event(
            HarnessEventType::IterationStart,
            1,
            "cycle 1/3 — task-1: think: no tools [budget 0/60000 tokens]",
            None,
        ),
...
fn warnings_and_errors_stay_visible_and_flush_groups() {
    ...
    assert_eq!(
        group_count(&p),
        2,
        "warning/error are boundaries, not group members"
    );
    let rows = compact_rows(&p, 120);
...
fn replay_with_unfinished_final_group_matches_incremental_feed() {
    ...
    assert_eq!(
        incremental.cells, replay.cells,
        "live and replay must agree"
    );
    assert_eq!(incremental.is_active(), replay.is_active());
    assert!(
        replay.is_active(),
        "unfinished final group stays open for the live row"
    );
    ...
    assert_eq!(
        last_group(&incremental.cells).tools,
        vec!["READ", "PATCH", "GREP"]
    );
}
...
fn narrow_terminals_clip_rows_ansi_safely() {
    let mut p = CompactProjection::new();
    p.append(&goal(
        "fix the ☕ café test — a very long unicode goal text",
    ));
    p.append(&tool_call(
        1,
        "READ",
        "c1",
        "{\"path\":\"src/ünïcödé/päth.rs\"}",
    ));
    p.append(&tool_call(1, "PATCH", "c2", "{}"));
    p.append(&event(
		HarnessEventType::IterationStart,
...
fn dripw_view_still_shows_raw_tool_rows_and_telemetry() {
    ...
    assert_eq!(
        rows.iter().filter(|r| r.contains("] tool ")).count(),
        2,
        "{rows:?}"
    );
    assert_eq!(
        rows.iter().filter(|r| r.contains("] result ")).count(),
        2,
        "{rows:?}"
    );
    assert_eq!(
        rows.iter().filter(|r| r.contains("] infer ")).count(),
        1,
        "{rows:?}"
    );
    assert!(
        rows.iter().any(|r| r.contains("READ src/lib.rs")),
        "{rows:?}"
    );
    assert!(
        rows.iter().any(|r| r.contains("PATCH src/lib.rs")),
        "{rows:?}"
    );
    assert!(rows.iter().any(|r| r.contains("result text")), "{rows:?}");
